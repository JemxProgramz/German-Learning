...
content = content.replace(/\{sessionScore\.total > 0 && \([\s\S]*?\)\}/, `{sessionScore.total > 0 && (
...
// Need to import Zap if not imported
content = content.replace("import { Play, RotateCcw, Heart }", "import { Play, RotateCcw, Heart, Zap }");
